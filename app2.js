...
function dibujarRuta(salida, destino) {
    const points = [[salida.lat, salida.long], [destino.lat, destino.long]];
...
function actualizarFilaVuelo(plane) {
    const { flight_id, captain, ETA, status } = plane;
    ...
    if (filaExistente) {
        ...
        const celdaEstado = filaExistente.querySelector('.estado');
        if (celdaEstado.textContent !== status) {
            celdaEstado.textContent = status;
        }
    }
}
